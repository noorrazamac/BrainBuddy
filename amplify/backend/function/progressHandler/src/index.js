...
async function getItemFromDynamoDB(event) {
    if ('queryStringParameters' in event) {
        ...
        if (pathParameters && 'student_id' in pathParameters) {
            ...
            console.log("fetching data for student's progress: ", student_id);
            const params = {
                TableName: process.env.STORAGE_PROGRESS_NAME,
                // TableName: "student-" + process.env.ENV,
                Key: {
                    student_id: { S: student_id } // Assuming the primary key is of type string
                }
            };
            try {
    ...
    if ('body' in event) {
        ...
        if (body && 'student_id' in body) {
            ...
            const jsonObject = {
                ...
                courses: "courses" in body?body["courses"]:[]
